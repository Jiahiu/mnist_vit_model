...
class ViT(nn.Module):
    def __init__(self, emb_size=16):
        ...
        self.patch_emb = nn.Linear(
            in_features=self.patch_size**2, out_features=emb_size
        )  # patch做emb
        self.cls_token = nn.Parameter(torch.rand(1, 1, emb_size))  # 分类头输入
        self.pos_emb = nn.Parameter(
            torch.rand(1, self.patch_count**2 + 1, emb_size)
        ...
        self.cls_linear = nn.Linear(
            in_features=emb_size, out_features=10
        )  # 手写数字10分类
    ...
    def forward(self, x):  # (batch_size,channel=1,width=28,height=28)
        ...
        x = x.view(
            x.size(0), x.size(1), self.patch_count**2
        )  # (batch_size,channel=16,seq_len=49)
